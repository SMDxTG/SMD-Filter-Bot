...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_text("<pre>ʜᴇʏ ᴅᴜᴅᴇ <b>sᴍᴅʙᴏᴛᴢ</b>.\nʜᴏᴘᴇ ʏᴏᴜ'ʀᴇ ᴅᴏɪɴɢ ᴡᴇʟʟ...</pre>")
        await asyncio.sleep(0.4)
        await m.edit_text("🕊️")
        await asyncio.sleep(0.5)
        await m.edit_text("🦋")
        await asyncio.sleep(0.5)
        await m.edit_text("<pre><i>ꜱᴛᴀʀᴛɪɴɢ sᴍᴅ ʙᴏᴛ...</i></pre>")
        await asyncio.sleep(0.4)
        await m.delete()        
        m=await message.reply_sticker("CAACAgUAAxkBAAEMF3lmdyvoJS0Jzd6yHzrlc2vf5LFHRQACfRAAAkzc-VZv3gYX-6y80B4E") 
